...
router.get('/:id', (req, res, next) => {
    ...
    if (customerId != null) {
        req.getConnection(function (err, conn) {
            ...
            conn.query("SELECT * FROM companies WHERE id=?", [1], function (err, company, fields) {
                if (err) {
                    res.status(500)
                    ...
                    if (company.length > 0) {
                        ...
                        const docDefinitions = {
                            ...
                            content: [
                                { text: "Meu primeiro relatorio da TsenaneApp" },
                            ]
                        }

                        const currentDate = new Date().getTime()
                        const pdfDoc = printer.createPdfKitDocument(docDefinitions)
                        // pdfDoc.pipe(fs.createWriteStream("relatorio_123.pdf"))
