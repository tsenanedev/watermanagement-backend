...
router.get("/", jwtMW, (req, res, next) => {
  ...
  if (uid != null) {
      ...
      conn.query(
        ...
        function (err, rows, fields) {
          if (err) {
            ...
            const abnormalCustomers = [];
            rows.forEach((row) => {
              const barcode = row.barcode;
              conn.query(
                "SELECT * FROM readings WHERE barcode=? AND subtotal=?",
                [barcode, 160],
                (err, readings, fields) => {
                  if (readings.length > 6) {
                    console.log(
                      `${readings.length}, ${row.barcode}`,
                      `, ${row.name}`
                    );
                  }
                }
            ...
            res.send(
              JSON.stringify({
                success: true,
                abnormalCustomers: abnormalCustomers,
              })
            );
          }
        }
    ...
    res.send(
      JSON.stringify({ success: false, message: "Invalid requellllst" })
    );
  }
});
...
router.post("/", jwtMW, (req, res, next) => {
  ...
  if (uid != null) {
    if (id > 0) {
      ...
      req.getConnection(function (err, conn) {
        conn.query(
          "INSERT INTO branches(provinceId, districtId, name, companyId, createdAt) VALUES(?,?,?,?,?)",
          [provinceId, districtId, name, companyId, createdAt],
          function (err, rows, fields) {
            if (err) {
              res.status(500);
              res.send(
